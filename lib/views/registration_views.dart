...
class RegistrationPage extends StatelessWidget {
      ...
      child: AnimatedPadding(
        ...
        child: Column(
          children: <Widget>[
            Column(
              children: <Widget>[
                Text('Sign up', style: TextStyle(fontSize: 25.0)),
  ...
  _register(BuildContext context, LoginBloc bloc) async {
    Map info =
        await userProvider.register(bloc.email.trim(), bloc.password.trim());

    if (info['ok']) {
      Navigator.pushReplacementNamed(context, 'info');
    } else {
      utils.showAlert(context, 'USER ALREADY EXISTS');
    }
  }
}
